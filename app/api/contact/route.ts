  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      return NextResponse.json(
        { success: false, message: "Zbyt wiele żądań. Spróbuj ponownie później." },
        { status: 429 }
      );
    }
    ...
    try {
      ...
      const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Pasieka Staropolska <kontakt@pasiekastaropolska.pl>",
          to: [process.env.CONTACT_EMAIL_TO || "kontakt@pasiekastaropolska.pl"],
          reply_to: email,
          subject: `[Pasieka Staropolska] Nowa wiadomość od ${escapeHtml(name)}`,
          html: `
            <div style="font-family: sans-serif; padding: 20px; color: #111; line-height: 1.6;">
              <h2 style="color: #b45309;">Nowa wiadomość z formularza kontaktowego</h2>
              ...
              <p style="white-space: pre-wrap; background: #fdfbf7; padding: 15px; border-radius: 8px; border: 1px solid #f2e8dc;">${escapeHtml(message)}</p>
            </div>
          `,
        }),
      });
      ...
      if (isDev) {
        console.warn("DEV_MODE WARNING: Sieć zablokowała fetch do Resend. Symuluję sukces wysyłki dla UI.");
